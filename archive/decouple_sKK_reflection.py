...
def layers_to_uniform(d_list, e_list, dx):
    """
    Convert piecewise-constant layers (thicknesses d_list, permittivities e_list)
    into a uniform grid suitable for FFT.

    Returns:
      x_uniform: 1D array of grid points in [0, L)
      eps_uniform: complex ε(x) sampled on the uniform grid
      L: total thickness
      dx: the grid spacing you passed in
    """
    d = np.asarray(d_list, dtype=float)
    e = np.asarray(e_list, dtype=complex)
    assert d.ndim == 1 and e.ndim == 1 and len(d) == len(e), "d_list and e_list must match"

    L = np.sum(d)
    # grid points in [0, L), length N so that x[n] = n*dx
    N = int(np.floor(L / dx))
    x_uniform = np.arange(N, dtype=float) * dx
    eps_uniform = np.empty(N, dtype=complex)

    # fill each interval with its layer value
    pos = 0.0
    idx0 = 0
    for dj, ej in zip(d, e):
        pos_next = pos + dj
        idx1 = min(N, int(np.floor(pos_next / dx)))
        if idx1 > idx0:
            eps_uniform[idx0:idx1] = ej
        pos = pos_next
        idx0 = idx1

    # if rounding left a small tail, fill it with the last layer’s value
    if idx0 < N:
        eps_uniform[idx0:] = e[-1]

    return x_uniform, eps_uniform, L

# ---- example usage ----
# x_uniform, eps_uniform, L, dx = layers_to_uniform(d_list, e_list, dx)
...
n_list, d_list = tmm_h.generate_n_and_d_v6_symmetry(gam, A, nb, delta=0.2, plot_flag=False, zoomed=True)

e_list = np.square(n_list)
dx = np.min(d_list)/8

x_uniform, eps_uniform, L = layers_to_uniform(d_list, e_list, dx)

xlabel = 'Wavelength ($\mu$m)'; ylabel = 'Fraction of Power'
title = f'TRA (A={A}, x$_0$={gam}$\mu$m), 25 Layers'
fig,ax = plot_setup(xlabel,ylabel,title=title,xlim=(x_uniform[0],x_uniform[-1]),figsize=(5,4),auto_scale=True)

plot(fig,ax,x_uniform,eps_uniform,label=r'T',color=colors.blue,auto_scale=True)
...
T_list_LR, R_list_LR, A_list_LR = tmm_h.TRA_angle(n_list, d_list, angle_list=angle_list_rad, pol='p')
T_list_RL, R_list_RL, A_list_RL = tmm_h.TRA_angle(n_list_reversed, d_list_reversed,angle_list=angle_list_rad, pol='p') 
