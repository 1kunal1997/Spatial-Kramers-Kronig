...
# ht_original() — moved from skk_analysis_consolidated.py
def ht_original(xx, e_re, pad_factor=4, taper_frac=0.05):
    """Hilbert transform with padding + Tukey taper (baseline method)."""
    N = len(e_re)
    eL = np.mean(e_re[:max(4, N//100)])
    eR = np.mean(e_re[-max(4, N//100):])
    pad = pad_factor * N
    e_pad = np.concatenate([np.full(pad, eL), e_re, np.full(pad, eR)])
    M = len(e_pad)
    taper = tukey(M, alpha=2*taper_frac)
    e_pad2 = eL + (e_pad - eL) * taper
    z = hilbert(e_pad2)
    return np.imag(z)[pad:pad+N]
