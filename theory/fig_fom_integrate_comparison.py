"""
fig_fom_integrate_comparison.py
===============================
2x2 comparison of spectral one-sidedness FoM — no zero-padding anywhere.

  Top left:     Lorentzian — direct FT of z(x) = dε'/dx + i·dε''/dx
  Top right:    Lorentzian — derivative → FT → ÷ik (recovers ε̂(k))
  Bottom left:  GRIN logistic — direct FT of z(x) = dε'/dx + i·dε''/dx
  Bottom right: GRIN logistic — derivative → FT → ÷ik (recovers ε̂(k))
"""
...
def compute_fom_from_spectrum(k, pwr):
    """Compute FoM from a power spectrum array (k, |Z|^2)."""
...
def deriv_ft_integrate(x, u, v):
    """Derivative → FT → ÷ik to recover ε̂(k)."""
    x = np.asarray(x, float)
...
print(f"(c) GRIN,       direct FT:            FoM = {fom_c:.2f}%")
print(f"(d) GRIN,       deriv-FT-integrate:    FoM = {fom_d:.2f}%")

# === 2x2 Plot ===
fig, axes = plt.subplots(2, 2, figsize=(13, 10))

configs = [
    (axes[0, 0], k_a, pwr_a, fom_a, 50,
     r'(a) Lorentzian — direct FT of $\varepsilon(x)$'),
    (axes[0, 1], k_b, pwr_b, fom_b, 50,
     r'(b) Lorentzian — $d\varepsilon/dx \to$ FT $\to \div\, ik$'),
    (axes[1, 0], k_c, pwr_c, fom_c, 400,
     r'(c) Logistic — direct FT of $\varepsilon(x)$'),
    (axes[1, 1], k_d, pwr_d, fom_d, 400,
     r'(d) Logistic — $d\varepsilon/dx \to$ FT $\to \div\, ik$'),
]

for ax, k, pwr, fom, klim, title in configs:
    # Normalize: exclude |k| < 1 to avoid DC/near-DC spike dominating
...
outpath = os.path.join(FIGDIR, 'fig_fom_integrate_comparison.png')
plt.savefig(outpath)
plt.close()
print(f"\nSaved: {outpath}")
