...
n_list, d_list = tmm_h.generate_n_and_d_v6_symmetry(gam, A, nb, delta=0.1, plot_flag=True, zoomed=False)
...
n_coating = n_list[max_index:min_index+1]
#n_coating = n_list
n_coating = [num.real + 1j*num.imag*0.1 for num in n_coating]
#d_coating = d_list
d_coating = d_list[max_index:min_index+1]
for i, n in enumerate(n_coating):
    print(n)
...
for j, angle in enumerate(angle_list*degrees):
    vw_list.append(tmm.inc_tmm('p', n_total, d_total, c_total, angle, lamb)['VW_list'])
